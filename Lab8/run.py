...
app = Flask(__name__)
app.debug = True
data = pd.read_csv('data1.csv',sep=',',usecols=['x','y'])
x = data['x'].tolist()
y = data['y'].tolist()
...
def home():
    ...
    return render_template('base.html',data=data_new,fit=fit,r=r,se=se,r_square=r_square,SSResid=SSResid,SSTo=SSTo)
